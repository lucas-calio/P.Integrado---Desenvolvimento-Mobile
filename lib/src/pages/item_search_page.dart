...
/// - Carrinho de impressão com bottom sheet e swipe-to-dismiss
/// - Detalhes completos do item (estoque, preços, depósitos, dimensões)
...
class _ItemSearchPageState extends State<ItemSearchPage> {
  ...
  /// Abre a [StoxScannerPage] e usa o código lido como termo de busca.
  Future<void> _abrirScanner() async {
    ...
    if (codigo != null && codigo.isNotEmpty && mounted) {
      setState(() => _searchController.text = codigo);
      _buscar();
    }
  }
  ...
  Widget _buildDetalheItem() {
    ...
    final qtd =
        num.tryParse(_itemData!['QuantityOnStock']?.toString() ?? '0') ?? 0;
    final minimo =
        num.tryParse(_itemData!['MinInventory']?.toString() ?? '0') ?? 0;
    final maximo =
      ...
      child: ListView(
        ...
        children: [
          ...
          StoxSectionCard(
            ...
            linhas: [
              StoxDetailRow('Unidade de Medida',
                  _itemData!['InventoryUOM']?.toString()),
              StoxDetailRow('Embalagem',
                  _itemData!['SalesPackagingUnit']?.toString()),
              StoxDetailRow('Código de Barras (EAN)',
  ...
  Widget _buildInfoDepositos() {
    ...
    final depositos =
        lista.where((wh) => (wh['InStock'] ?? 0) > 0).toList();

    if (depositos.isEmpty) {
      return Padding(
        padding: const EdgeInsets.only(bottom: 8),
        child: Text(
          'Sem estoque disponível em nenhum depósito.',
  ...
  String? _formatPrecoLista(int lista) {
    ...
    return _formatPreco(prices[index]['Price']);
  }
